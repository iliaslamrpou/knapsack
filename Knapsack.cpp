/* KnapsackSolver ver 1.0.3
   Created by: Ηλίας Λάμπρου
   Δημιουργήθηκε: 15/12/2019
   Τροποποιήθηκε: 31/12/2019
...
struct item
{
    ...
    int profit;     // αξία
    int weight;     // βάρος
    int bound;      // θα χρησιμοποιηθεί από τον αλγόριθμο  Branch and Bound
    int level;      // θα χρησιμοποιηθεί από τον αλγόριθμο  Branch and Bound
...
std::vector <item> brute_force_solver(knapsack_problem& ks) {
   ...
   // std::cout<<"=======================================\n";
    ...
    std::vector<item> sol;
    for (int i = 0;i < total;i++){    //  Tα bits του i που θα είναι 1 θα περιέχουν τους συνδιασμούς τους αντικειμένων αντίστοιχα με την θέση τους στο 32bit αριθμό (από δεξιά)
        sol.clear();
...
std::vector <int>  branch_and_bound_action2(int W, item arr[], int n) {
    ...
    while (!Q.empty()) {        // Η ουρά περιέχει τα αντικείμενα που πρέπει να ελεγθούν. Εφόσον δεν είναι άδεια συνεχίζουμε
        u = Q.front();          // Κρατάμε στη u το πρώτο αντικείμενο από την ουρά
        Q.pop();                // και μετά το αφαιρούμε από την ουρά αφού θα ελεγθεί παρακάτω 
        if (u.level == -1) v.level = 0;    // Την πρώτη φορά που θα εκτελεσθεί το loop η u.level είναι -1 και την κάνουμε 0
        if (u.level == n - 1) continue;    // εάν φτάσουμε στο τελευταίο αντικείμενο σταματάμε και πάμε να εξετάσουμε το επόμενο της ουράς

        v.level = u.level + 1;  // Ο έλεγχος θα γίνει στο επόμενο level (αντικείμενο) από αυτό που τραβήξαμε από την ουρά
        ...
        v.id = arr[v.level].id;
        v.itemList.clear();                           
        v.itemList = u.itemList;        // κάθε στοιχείο κρατάει και μια λίστα με όλα τα επιλεγμένα αντικείμενα του κλάδου του μέχρι τώρα
        v.itemList.push_back(v.id);     // και σε αυτή τη λίστα προσθέτουμε και το τρέχον αντικείμενο αφού το επιλέγουμε ως μέρος τηε λύσης

        if (v.weight <= W && v.profit > maxProfit) {  // εάν το βάρος δεν έχει ξεπεράσει το μέγιστο βάρος και η αξία του δεν είναι μικρότερη από αυτή που μέχρι τώρα έχουμε ως καλύτερη λύση
        ...
        if (v.bound > maxProfit) {                    // αν η μέγιστη αξία είναι καλύτερη από αυτή που έχουμε
            Q.push(v);                                // το v εισάγεται στην ουρά ώστε κάποια στιγμή στη συνέχεια να ελεγθουν οι δύο περιπτώσεις για το επόμενο από αυτό αντικείμενο
        }
        ...
        v.profit = u.profit;                          
        v.id = u.id;
        v.itemList = u.itemList;                      // οι λύσεις που θα δημιουργηθούν δεν θα συμπεριλαμβάνουν το τρέχον αντικείμενο  
...
std::vector <item>  branch_and_bound(knapsack_problem& ks) {
    ...
    int n = ks.items.size();
    item *arr = new item[n];
    for (int i = 0;i < n;i++) arr[i] = ks.items[i];
...
/*=============================================================== printOneProblemSolution
  ...
  Αποθηκεύει την λύση σε αρχείο πχ solution_10_50_1_2_5_a2.txt  όπου οι τελευταίοι χαρακτήρες του ονόματος καθορίζονται από τον αλγόριθμο επίλυσης
...
void showMenu(int color) {
    ...
    std::cout << char(177); std::cout << char(177); std::cout << "               KnapsackSolverver 1.0.2                  ";std::cout << char(177); std::cout << char(177);std::cout << '\n';// delay(200);
    std::cout << char(177); std::cout << char(177); std::cout << "              Created by: Ilias Lamprou                 ";std::cout << char(177); std::cout << char(177);std::cout << '\n';// delay(200);
